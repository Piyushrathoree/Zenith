...
export default function Dashboard() {
  ...
  const [integrations, setIntegrations] = useState<IntegrationCard[]>(integrationCards)
  const [activeTask, setActiveTask] = useState<Task | null>(null)
  const [activeIntegration, setActiveIntegration] = useState<IntegrationCard | null>(null)
  const [isTaskModalOpen, setIsTaskModalOpen] = useState(false)
  const [selectedColumnId, setSelectedColumnId] = useState<string | null>(null)
  const [selectedTask, setSelectedTask] = useState<Task | IntegrationCard | null>(null)
  const [isDetailPanelOpen, setIsDetailPanelOpen] = useState(false)
  const [leftSidebarOpen, setLeftSidebarOpen] = useState(false)
  const [rightSidebarOpen, setRightSidebarOpen] = useState(false)
  const [currentView, setCurrentView] = useState<"kanban" | "today" | "focus" | "daily-planner">("kanban")
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    const sourceColumn = kanbanData.columns.find((col) => col.tasks.some((task) => task.id === activeId))

    let destColumn = kanbanData.columns.find((col) => col.id === overId)
    if (!destColumn) {
      destColumn = kanbanData.columns.find((col) => col.tasks.some((t) => t.id === overId))
    }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (activeIntegration) {
      const destColumn = kanbanData.columns.find((col) => col.id === overId || col.tasks.some((t) => t.id === overId))
      if (destColumn) {
        const newTask: Task = {
          id: `task-${Date.now()}`,
        ...
        setKanbanData((prev) => ({
          ...
          columns: prev.columns.map((col) =>
            col.id === destColumn.id ? { ...col, tasks: [...col.tasks, newTask] } : col,
          ),
        }))
        setIntegrations((prev) => prev.filter((i) => i.id !== activeId))
      }
      return
    }

    const column = kanbanData.columns.find((col) => col.tasks.some((task) => task.id === activeId))
    if (column) {
      ...
      if (oldIndex !== -1 && newIndex !== -1 && oldIndex !== newIndex) {
        setKanbanData((prev) => ({
          ...prev,
          columns: prev.columns.map((col) =>
            col.id === column.id ? { ...col, tasks: arrayMove(col.tasks, oldIndex, newIndex) } : col,
          ),
        }))
      }
    }
  ...
  const handleViewChange = (view: "kanban" | "today" | "focus" | "daily-planner") => {
  ...
  return (
    <ThemeProvider>
      ...
      >
            ...
            <AnimatePresence mode="wait">
              {currentView === "kanban" && (
                <motion.div
                  ...
                  className="h-full"
                >
                  <KanbanBoard data={kanbanData} onAddTask={handleAddTask} onTaskClick={handleTaskClick} />
                </motion.div>
              )}
              {currentView === "today" && (
                <motion.div
                ...
                >
                  <TodayView
                    ...
                    onAddTask={() => {
                      setSelectedColumnId(kanbanData.columns[0]?.id || null)
                      setIsTaskModalOpen(true)
                    }}
                  />
                </motion.div>
              ...
              {currentView === "focus" && (
                <motion.div
                  ...
                  className="h-full"
                >
                  <FocusMode task={focusTask} onClose={() => setCurrentView("kanban")} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
        ...
        <DragOverlay dropAnimation={{ duration: 200, easing: "cubic-bezier(0.18, 0.67, 0.6, 1.22)" }}>
          {activeTask && (
            <motion.div
              ...
              animate={{ scale: 1.03, boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.25)" }}
              className="bg-card border border-border rounded p-3 w-[260px] cursor-grabbing"
            >
              <div className="flex justify-between items-start">
                <span className="text-xs font-medium">{activeTask.time}</span>
                <span className="text-[10px] bg-secondary px-1.5 py-0.5 rounded">{activeTask.duration}</span>
              </div>
              <p className="text-[13px] font-medium mt-1.5">{activeTask.title}</p>
            </motion.div>
          )}
          {activeIntegration && (
            <motion.div
            ...
            >
              <p className="text-[13px] font-medium">{activeIntegration.title}</p>
            </motion.div>
          )}
        </DragOverlay>
        ...
        {/* Task Detail Panel */}
        <TaskDetailPanel isOpen={isDetailPanelOpen} onClose={() => setIsDetailPanelOpen(false)} task={selectedTask} />
      </DndContext>
    </ThemeProvider>
  )
}
