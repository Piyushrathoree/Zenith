  ...
  return (
    ...
    >
      <div className="flex items-start gap-2 mb-2">
        ...
        <p className="text-xs text-muted-foreground truncate flex-1">{issue.repository}</p>
        <span className={cn(
          "px-2 py-0.5 text-xs rounded-full font-medium flex items-center gap-1",
          issue.state === 'open' ? "bg-status-open/15 text-status-open" : "bg-status-closed/15 text-status-closed"
        )}>
          <span className={cn("w-1.5 h-1.5 rounded-full", issue.state === 'open' ? "bg-status-open" : "bg-status-closed")} />
      ...
      <p className="text-xs text-muted-foreground mb-3">
        #{issue.number} · {formatDistanceToNow(new Date(issue.createdAt), { addSuffix: true })} · {issue.author}
      </p>
