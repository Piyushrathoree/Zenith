import { Home, Calendar, Target, UserPlus, ChevronDown, ChevronRight, CalendarCheck, TrendingUp } from 'lucide-react';
  ...
  return <>
      <aside className="w-sidebar h-screen flex flex-col bg-card border-r border-border overflow-hidden">
        ...
        <nav className="p-3 space-y-1">
          {navItems.map(item => <motion.button key={item.label} onClick={item.onClick} whileHover={{
          scale: 1.01
        }} whileTap={{
          scale: 0.99
        ...
        <div className="px-4 py-3">
          ...
          <div className="space-y-1">
            {dailyRituals.map(ritual => <button key={ritual.id} onClick={ritual.onClick} className="flex items-center gap-3 w-full px-2 py-2 rounded-lg hover:bg-muted transition-colors group">
                <ChevronRight className="w-4 h-4 text-muted-foreground group-hover:text-accent transition-colors" />
                <span className="text-sm text-foreground flex-1 text-left">{ritual.label}</span>
              </button>)}
          </div>
        </div>
        ...
        <div className="px-4 py-3">
          ...
          <div className="space-y-2">
            {weeklyRituals.map(ritual => <motion.button key={ritual.id} onClick={ritual.onClick} whileHover={{
            scale: 1.01
          }} whileTap={{
            scale: 0.98
                ...
                <div className="flex-1 text-left">
                  <p className="text-sm font-medium text-foreground">{ritual.label}</p>
        ...
        <div className="p-4 border-t border-border">
          <button className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg border border-border hover:bg-muted transition-colors">
            ...
            <span className="text-sm text-muted-foreground">Invite someone</span>
          </button>
        </div>
      </aside>

      {/* Weekly Rituals Panel */}
      <WeeklyRitualsPanel isOpen={showWeeklyRituals} onClose={() => setShowWeeklyRituals(false)} type={weeklyRitualType} />
    </>;
}