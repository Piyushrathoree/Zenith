...
import { DailyPlannerPanel } from '@/components/panels/DailyPlannerPanel';
import { WeeklyRitualsPanel } from '@/components/panels/WeeklyRitualsPanel';
...
export function DashboardLayout() {
  const { activeIntegration, focusMode, moveTask, addTask, columns, showWeeklyRituals, setShowWeeklyRituals, weeklyRitualType } = useApp();
      ...
      <DragOverlay dropAnimation={{ duration: 200, easing: 'cubic-bezier(0.18, 0.67, 0.6, 1.22)' }}>
            ...
            <div className="flex items-start gap-3">
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  {activeTask.time && (
                    <span className="text-xs font-medium text-muted-foreground">{activeTask.time}</span>
                  )}
                  <span className="text-xs px-2 py-0.5 bg-muted rounded text-muted-foreground">
                    {activeTask.duration}
                  </span>
                </div>
                <h4 className="text-sm font-medium text-foreground mb-1.5">{activeTask.title}</h4>
                <span className="text-xs font-medium tag-work">#{activeTask.tag}</span>
              </div>
            </div>
          </div>
        )}
        {activeDragType === 'github-issue' && activeDragData?.issue && (
          <div className="p-3 bg-card border border-accent rounded-lg shadow-xl w-72 rotate-2">
            <h4 className="text-sm font-medium text-foreground">{activeDragData.issue.title}</h4>
            <p className="text-xs text-muted-foreground mt-1">#{activeDragData.issue.number}</p>
          </div>
        )}
        {activeDragType === 'github-pr' && activeDragData?.pr && (
          <div className="p-3 bg-card border border-accent rounded-lg shadow-xl w-72 rotate-2">
            <h4 className="text-sm font-medium text-foreground">{activeDragData.pr.title}</h4>
            <p className="text-xs text-muted-foreground mt-1">#{activeDragData.pr.number}</p>
          </div>
        )}
        {activeDragType === 'gmail' && activeDragData?.message && (
          <div className="p-3 bg-card border border-accent rounded-lg shadow-xl w-72 rotate-2">
            <h4 className="text-sm font-medium text-foreground">{activeDragData.message.subject}</h4>
            <p className="text-xs text-muted-foreground mt-1">{activeDragData.message.from}</p>
          </div>
        )}
        {activeDragType === 'notion' && activeDragData?.page && (
          <div className="p-3 bg-card border border-accent rounded-lg shadow-xl w-72 rotate-2">
            <h4 className="text-sm font-medium text-foreground">{activeDragData.page.title}</h4>
            <p className="text-xs text-muted-foreground mt-1">{activeDragData.page.workspace}</p>
          </div>
        )}
      </DragOverlay>
    </DndContext>
